...
async def security_check(state: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = PromptPayload(
        ...
        target_agent_id=_SECURITY_AGENT_ID,
        content=last_human,
    )
...
async def core_router(state: dict[str, Any]) -> dict[str, Any]:
    ...
    last_human = _extract_last_human_text(state.get("messages", []))
    payload = PromptPayload(
        source_agent_id=_SECURITY_AGENT_ID,
        target_agent_id=_CORE_AGENT_ID,
        content=last_human,
    )
...
async def agent_executor(state: dict[str, Any]) -> dict[str, Any]:
    ...
    last_human = _extract_last_human_text(state.get("messages", []))
    payload = PromptPayload(
        source_agent_id=_CORE_AGENT_ID,
    ...
    new_interrupts: list[InterruptRequest] = []
...
async def prompt_relay(state: dict[str, Any]) -> dict[str, Any]:
    ...
    last_human = _extract_last_human_text(state.get("messages", []))
    payload = PromptPayload(
        source_agent_id=state.get("current_agent", "system"),
        target_agent_id=_PROMPT_RELAY_AGENT_ID,
        content=last_human,
        metadata={
            "routing_history": state.get("routing_history", []),
...
async def telemetry_emit(state: dict[str, Any]) -> dict[str, Any]:
    ...
    frames: dict[str, TelemetryFrame] = state.get("telemetry_frames", {})
    agent_summaries: list[str] = []
    for agent_id, frame in frames.items():
        agent_summaries.append(
            f"  • {agent_id}: status={frame.status.value}, "
