...
from singularity.neural_core.github_tools import SWARM_TOOLS
...
async def generate_interjection_intent(client: genai.Client, node_name: str, system_prompt: str, history_parts: list[Any], verbose_mode: bool) -> tuple[dict[str, Any], int]:
    ...
    contents = [genai.types.Part.from_text(f"You are {node_name}. Here is the simulation history:\n")]
    contents.extend(history_parts)
    contents.append(genai.types.Part.from_text("\n\n" + prompt_text))
    ...
    try:
        ...
        logger.error(f"Error getting intent for {node_name}: {e}")
        ...
        while True:
            ...
            for agent, intent in zip(agents_config, intents):
                ...
                if intent.get("intent") == "INTERJECT":
                    ...
                    if reason:
                        await queue.put((-priority, fifo_counter, node_name, reason))
                        fifo_counter += 1
            ...
            if queue.empty():
                ...
                try:
                    ...
                    await queue.put((-10, fifo_counter, node_name, winner_message))
                    fifo_counter += 1
                except Exception as e:
