...
events, accumulates the latest :class:`TelemetryFrame` per agent, and
...
from singularity.core.agent_registry import get_all_agents
...
class TelemetryCollector:
    ...
    :class:`TelemetryFrame` snapshots emitted by agents and the
    ...
    def __init__(self, bus: TelemetryEventBus | None = None) -> None:
        ...
        self._latest_frames: dict[str, TelemetryFrame] = {}
    ...
    async def _on_heartbeat(self, event: TelemetryEvent) -> None:
        ...
        for frame_dict in frames_data:
            try:
                ...
                self._latest_frames[frame.agent_id] = frame
            except Exception:
    ...
    async def _on_error(self, event: TelemetryEvent) -> None:
        ...
        self._latest_frames[agent_id] = TelemetryFrame(
            agent_id=agent_id,
    ...
    def get_latest_frame(self, agent_id: str) -> TelemetryFrame | None:
        ...
        Returns:
            The latest :class:`TelemetryFrame`, or ``None`` if no frames
            have been received from this agent.
        """
        return self._latest_frames.get(agent_id)

    async def collect_live_telemetry(self) -> list[TelemetryFrame]:
        """Poll every registered agent for a fresh telemetry frame.

        Calls :meth:`AsyncBaseAgent.emit_telemetry` on each agent from
        the registry and updates the internal cache.

        Returns:
            List of freshly collected :class:`TelemetryFrame` instances.
        """
        ...
        for agent in agents:
            try:
                ...
                error_frame = TelemetryFrame(
                    agent_id=agent.agent_id,
                    status=AgentStatus.ERROR,
                    message="Telemetry collection failed",
                )
                self._latest_frames[agent.agent_id] = error_frame
    ...
    def get_constellation_status(self) -> dict[str, Any]:
        ...
        for agent in all_agents:
            cached_frame = self._latest_frames.get(agent.agent_id)
